###For the time being, These TP's will be dependent on the map's folder, in the future i plan to fix this

##Crucible
execute if score #Store 4j.map matches 1 run function 4jbattle:crucible/setup/teleport/surround

##Cove
execute if score #Store 4j.map matches 2 run function 4jbattle:cove/setup/teleport/surround

##Cavern
execute if score #Store 4j.map matches 3 run function 4jbattle:cavern/setup/teleport/surround

##Small Crucible
execute if score #Store 4j.map matches 4 run function 4jbattle:crucible_small/setup/teleport/surround

##Small Cove
execute if score #Store 4j.map matches 5 run function 4jbattle:cove_small/setup/teleport/surround

##Small Cavern
execute if score #Store 4j.map matches 6 run function 4jbattle:cavern_small/setup/teleport/surround

##Large Frontier
execute if score #Store 4j.map matches 7 run function 4jbattle:frontier/setup/teleport/surround

##Small Frontier
execute if score #Store 4j.map matches 8 run function 4jbattle:frontier_small/setup/teleport/surround
