...
execute if score #Store 4j.glow matches 1 run effect give @a[predicate=!4jbattle:is_sneaking,gamemode=!spectator,nbt=!{ActiveEffects:[{Id:14}]}] glowing 1 0 true

##Remove glowing effect from invisible players
effect clear @a[nbt={ActiveEffects:[{Id:14}]}] glowing
