##Add kills
execute as @a[tag=player] run scoreboard players operation @s 4j.drawscore = @s 4j.killcount
...
#Get Victor count
execute store result score #Store 4j.victorcount if entity @a[tag=Victor]
#Draw if nessecary
execute if score #Store 4j.victorcount matches 2.. run tag @a remove Victor
