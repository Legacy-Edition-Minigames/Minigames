##Add damage dealt
execute as @a[tag=ingame] run scoreboard players operation @s 4j.drawscore = @s 4j.damagedealt

##Remove damage taken
execute as @a[tag=ingame] run scoreboard players operation @s 4j.drawscore -= @s 4j.damagetaken
...
execute as @a[tag=ingame] if score @s 4j.drawscore = #highestScore 4j.drawscore run tag @s add Victor
